...
void solve() {
    int n, m;
    cin >> n >> m;
    map<int, int> donors, acceptors;
    while (m--) {
        int a, b, c;
        cin >> a >> b >> c;
        donors[a] += c, acceptors[b] += c;
    }
    deque<int> givers, receivers;
    for (auto donor: donors) {
        if (!acceptors.contains(donor.fi)) {
            givers.pp(donor.se);
            continue;
        }
        int val = donor.se - acceptors[donor.fi];
        acceptors.erase(donor.fi);
        if (val > 0)
            givers.pp(val);
        else
            receivers.pp(-val);
    ...
    for (auto acceptor: acceptors) {
        receivers.pp(acceptor.se);
    }

    sort(rall(givers)), sort(rall(receivers));
    int sum = 0;
    for (auto giver: givers) {
        sum += giver;
        for (auto &receiver: receivers) {
            if (giver >= receiver) receivers.pop_front();
            else {
                receiver -= giver;
                break;
            }
