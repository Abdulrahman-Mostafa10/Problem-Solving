...
void solve() {
    ...
    lpato(points) cin >> it.fi.fi >> it.fi.se >> it.se;

    int cnt = 0;
    lp(k, 2) {
        int row = (!k ? min(x1, x2) : max(x1, x2));
        lpi(i, min(y1, y2), max(y1, y2) + 1) {
            bool f = 0;
            for (auto point: points) {
                if (f) break;
                if ((((point.fi.fi - row) * (point.fi.fi - row)) + ((point.fi.se - i) * (point.fi.se - i))) <=
                    point.se * point.se)
                    f = 1;
            }
            cnt += !f;
    ...
    lp(k, 2) {
        int col = (!k ? min(y1, y2) : max(y1, y2));
        lpi(i, min(x1, x2) + 1, max(x1, x2)) {
            bool f = 0;
            for (auto point: points) {
                if (f) break;
                if ((((point.fi.fi - i) * (point.fi.fi - i)) + ((point.fi.se - col) * (point.fi.se - col))) <=
                    point.se * point.se)
                    f = 1;
            }
            cnt += !f;
