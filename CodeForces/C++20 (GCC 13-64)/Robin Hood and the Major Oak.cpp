...
ll calc(int exp, int b) {
    ll res = 1;
    while (exp > 0) {
        if (exp % 2) res = (b * res) % mod;
        b = (b * b) % mod;
        exp /= 2;
    }
    return res;
...
void solve() {
    ll n, k;
    cin >> n >> k;
    ll firstLeaves = max(1LL, n - k + 1), lastLeaves = n, total =
            (lastLeaves * (lastLeaves + 1)) / 2 - (firstLeaves - 1) * firstLeaves / 2;
    cout << (total % 2 ? "No" : "Yes") << endl;
}
