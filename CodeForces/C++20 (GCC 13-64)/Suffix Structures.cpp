...
string analyze(string &s, const string &t) {
    ...
    if (canBeSuffixed) {
        for (auto c: sFreq) {
            if (tFreq.contains(c.fi) && tFreq[c.fi] < c.se || s.length() > t.length()) {
                canBeSuffixed = false;
                break;
            }
        }
    ...
    if (canBeSuffixed) {
        ...
        for (auto c: sFreq) {
            if (tFreq.contains(c.fi) && tFreq[c.fi] > c.se) {
                return "need tree";
            }
        }
        string old = s;
        ...
        lpi(i, 1, t.size()) {
            st = s.find(t[i], st + 1);
            if (st == string::npos) {
                if (tFreq.size() == sFreq.size() && old.length() == t.length())
                    return "array";
                return "both";
            }
        }
