...
void solve() {
    int n;
    cin >> n;
    map<int, pair<bool, pii>> mp;
    lp(i, n) {
        int x;
        cin >> x;
        if (!mp.contains(x))
            mp[x] = make_pair(false, make_pair(i, i));
        else {
            int diff = i - mp[x].se.fi;
            if (!mp[x].fi || mp[x].se.se == diff)
                mp[x] = make_pair(true, make_pair(i, diff));
            else
                mp[x] = make_pair(true, make_pair(i, 0));
        }
    }
    vi toDelete;
    for (auto &i: mp) {
