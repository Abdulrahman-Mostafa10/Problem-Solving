...
void solve() {
    ...
    cin >> n;
    pii origin;
    cin >> origin.fi >> origin.se;
    vpii v(n);
    lpato(v) cin >> it.fi >> it.se;
    vector<bool> vis(n);
    int cnt = 0;
    lp(i, n) {
        if (!vis[i]) {
            cnt++;
            lpi(j, i + 1, n) {
                int deltaX = v[i].fi - origin.fi, deltaY = v[i].se - origin.se, ddx = v[j].fi - v[i].fi, ddy =
                        v[j].se - v[i].se;
                if (!vis[j] && deltaX * ddy == deltaY * ddx) vis[j] = 1;
            }
        }
    }
    cout << cnt;
